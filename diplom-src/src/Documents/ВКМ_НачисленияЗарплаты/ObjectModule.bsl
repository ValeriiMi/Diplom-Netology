...
Процедура СформироватьДвиженияУдержаний()
	...
	|	ВЫБОР
	|		КОГДА ВКМ_УдержанияБазаВКМ_ОсновныеНачисления.ВидРасчетаРазрез = &Проценты
	|			ТОГДА МИНИМУМ(ВКМ_УдержанияБазаВКМ_ОсновныеНачисления.БазовыйПериодНачало)
	|		ИНАЧЕ МАКСИМУМ(ВКМ_УдержанияБазаВКМ_ОсновныеНачисления.БазовыйПериодНачало)
	|	КОНЕЦ КАК БазовыйПериодНачало,
	|	ВЫБОР
	|		КОГДА ВКМ_УдержанияБазаВКМ_ОсновныеНачисления.ВидРасчетаРазрез = &Проценты
	|			ТОГДА МАКСИМУМ(ВКМ_УдержанияБазаВКМ_ОсновныеНачисления.БазовыйПериодКонец)
	|		ИНАЧЕ МИНИМУМ(ВКМ_УдержанияБазаВКМ_ОсновныеНачисления.БазовыйПериодКонец)
	...
	Пока Выборка.Следующий() Цикл
		...
		Движение.Источник = Выборка.ВидРасчетаРазрез;
